...
export function WaitlistPanel() {
  ...
  if (loading || !user) {
    return <main className="grid min-h-screen place-items-center bg-[#f5f7f2] text-sm text-gray-500">Loading...</main>;
  }

  if (user.status === "active") {
    return (
      <main className="grid min-h-screen place-items-center bg-[#f5f7f2] px-4">
    ...
    <main className="min-h-screen bg-[#f5f7f2] text-ink">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex items-center justify-between">
          ...
          <button
            className="flex h-9 items-center gap-2 rounded-md border border-[#d8ddd2] bg-white px-3 text-sm font-medium text-gray-700 hover:bg-gray-50"
            onClick={logout}
          >
            <LogOut size={15} />
            Log out
        ...
        <section className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[1fr_0.85fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#d8ddd2] bg-white px-3 py-1 text-sm font-medium text-gray-700">
              <Sparkles size={15} />
              Waitlist confirmed
            </div>
            ...
            <div className="mt-8 grid gap-3 text-sm text-gray-700 sm:grid-cols-3">
              <div className="rounded-lg border border-[#d8ddd2] bg-white p-4">
          ...
          <div className="rounded-lg border border-[#cbd3c3] bg-white p-4 shadow-xl">
            <div className="rounded-md bg-ink p-5 text-white">
              <div className="flex items-center justify-between border-b border-white/15 pb-4">
                <p className="font-semibold">Launch queue</p>
                <span className="rounded-full bg-[#e0ff8f] px-2 py-1 text-xs font-semibold text-ink">MVP</span>
              </div>
              <div className="mt-5 space-y-4">
                {["Identity ready", "Waitlist seat reserved", "Chat access pending"].map((item, index) => (
                  <div className="flex items-center gap-3" key={item}>
                    ...
                    <div className="h-2 flex-1 rounded-full bg-white/15">
                      <div
                        className="h-2 rounded-full bg-[#e0ff8f]"
                        style={{ width: index < 2 ? "100%" : "52%" }}
                      />
                    </div>
                    <span className="w-32 text-xs text-white/75">{item}</span>
                  </div>
                ))}
              </div>
